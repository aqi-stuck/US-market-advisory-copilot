...
def client():
    ...
    with patch("app.rag.pipeline.run_pipeline") as mock_pipeline, patch(
        "app.llm.clients.get_chat_client"
    ), patch("app.rag.retriever.retrieve_chunks"), patch(
        "app.vectorstore.qdrant_client.upsert_points"
    ), patch(
        "app.llm.embeddings.embed_text"
    ) as mock_embed, patch(
        "app.data.preprocess.chunking.chunk_text"
    ) as mock_chunk:
